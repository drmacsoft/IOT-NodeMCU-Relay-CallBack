...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if ((char)payload[0] == '1') {
    ...
    client.publish("outTopic", "Relay 1 is ON");// Turns the relay ON
    Serial.print("Relay 1 is ON"); // Prints to seriel
  }
  if ((char)payload[0] == '0') {
    client.publish("outTopic", "Relay 1 is OFF");// Turns the relay OFF                       
  ...
  if ((char)payload[0] == '3') {
    ...
    client.publish("outTopic", "Relay 2 is ON");// Turns the relay ON
    Serial.print("Relay 2 is ON"); // Prints to seriel
  }
  else if ((char)payload[0] == '2') {
    client.publish("outTopic", "Relay 2 is OFF");// Turns the relay OFF                          
  ...
  /******* Switch on the RELAY3 if 5 was received as a first character ************************/
    ...
    client.publish("outTopic", "Relay 3 is ON");// Turns the relay ON
    Serial.print("Relay 3 is ON"); // Prints to seriel
  }
  else if ((char)payload[0] == '4') {
    client.publish("outTopic", "Relay 3 is OFF");// Turns the relay OFF
  ...
  /******* Switch on the RELAY4 if 7 was received as a first character ************************/
    ...
    client.publish("outTopic", "Relay 4 is ON");// Turns the relay ON
    Serial.print("Relay 4 is ON"); // Prints to seriel
  }
  else if ((char)payload[0] == '6') {
    client.publish("outTopic", "Relay 4 is OFF");// Turns the relay OFF                         
